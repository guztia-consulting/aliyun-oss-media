...
<div class="as3cf-content as3cf-provider-select">
	...
	<table>
		<?php
		/* @var \GuztiaConsulting\Aliyun_OSS_Media\Providers\Storage\Storage_Provider $provider_class */
		foreach ( $providers as $provider_key => $provider_class ) {
			/* @var \GuztiaConsulting\Aliyun_OSS_Media\Providers\Storage\Storage_Provider $provider */
			...
			<tr class="as3cf-provider-title as3cf-provider-<?php echo $provider_key; ?><?php echo $provider_selected_class; ?>">
				...
				<td>
					...
					if ( $provider_selected && $provider_defined ) {
						echo '<span class="as3cf-defined-in-config">' . __( 'defined in theme', 'amazon-s3-and-cloudfront' ) . '</span>';
					}
					?>
					...
					<table>
						...
						if ( $provider_class::use_access_keys_allowed() ) {
							...
							<tr class="asc3f-provider-authmethod-title">
								...
								<td>
									<label for="as3cf-provider-<?php echo $provider_key; ?>-define"><?php _e( 'Define access keys in your theme', 'amazon-s3-and-cloudfront' ) ?></label>
								</td>
							</tr>
								...
								<td>
									...
									if ( $any_access_key_constant_defined ) {
										if ( count( $defined_constants ) > 1 ) {
											$remove_defines_msg = _x( 'You\'ve defined your access keys in your theme.', 'Access Keys defined in multiple defines.', 'amazon-s3-and-cloudfront' );
										} else {
											$remove_defines_msg = _x( 'You\'ve defined your access keys in your theme.', 'Access Keys defined in single define.', 'amazon-s3-and-cloudfront' );
										}
										...
										if ( $provider_selected && ! $provider->are_access_keys_set() ) {
											...
											<div class="notice-error notice">
												<p>
													<?php _e( 'Please check your theme files as it looks like one of your access key defines is missing or incorrect.', 'amazon-s3-and-cloudfront' ) ?>
												</p>
											</div>
											<?php
										}
									} else {
										_e( 'Copy the following snippet to your theme and replace the stars with the keys.', 'amazon-s3-and-cloudfront' );
...
define( '<?php echo $this::preferred_settings_constant(); ?>', serialize( array(
    ...
    'secret-access-key' => '**************************************',
							...
							<tr class="asc3f-provider-authmethod-title">
								...
								<td>
									<label for="as3cf-provider-<?php echo $provider_key; ?>-define"><?php _e( 'Define key file path in theme', 'amazon-s3-and-cloudfront' ) ?></label>
								</td>
							</tr>
							<tr class="asc3f-provider-authmethod-content" data-provider-authmethod="define"<?php echo 'define' !== $selected_authmethod ? ' style="display: none"' : ''; ?>>
								<td></td>
								<td>
									<?php
									if ( $any_access_key_constant_defined ) {
										$remove_defines_msg         = _x( 'You\'ve defined your key file path in your theme. To select a different option here, simply comment out or remove the \'%1$s\' define in your theme.', 'Key file path defined in single define.', 'amazon-s3-and-cloudfront' );
										$multiple_defined_keys_glue = _x( ' & ', 'joins multiple define keys in notice', 'amazon-s3-and-cloudfront' );
										$defined_constants_str      = join( $multiple_defined_keys_glue, $defined_constants );
										printf( $remove_defines_msg, $defined_constants_str );
										echo '&nbsp;' . $this->more_info_link( '/wp-offload-media/doc/' . $provider_service_quick_start_slug . '/#save-key-file' );
									} else {
										_e( 'Copy the following snippet in your theme and replace "<strong>/path/to/key/file.json</strong>".', 'amazon-s3-and-cloudfront' );
										echo '&nbsp;' . $this->more_info_link( '/wp-offload-media/doc/' . $provider_service_quick_start_slug . '/#save-key-file' );
										?>
							...
							<tr class="asc3f-provider-authmethod-content" data-provider-authmethod="server-role"<?php echo 'server-role' !== $selected_authmethod ? ' style="display: none"' : ''; ?>>
								...
								<td>
									<?php if ( $any_use_server_roles_constant_defined ) {
										$remove_defines_msg         = _x( 'You\'ve defined use of server roles in your theme. To select a different option here, simply comment out or remove the \'%1$s\' define in your theme.', 'Use Server Roles defined in single define.', 'amazon-s3-and-cloudfront' );
										$multiple_defined_keys_glue = _x( ' & ', 'joins multiple define keys in notice', 'amazon-s3-and-cloudfront' );
