...
final class Env
{
    ...
    public static function search($expression, $data)
    {
        static $runtime;
        if (!$runtime) {
            $runtime = \GuztiaConsulting\Aliyun_OSS_Media\Aws3\JmesPath\Env::createRuntime();
        }
    ...
    public static function createRuntime()
    {
        switch ($compileDir = self::getEnvVariable(self::COMPILE_DIR)) {
            case false:
                return new \GuztiaConsulting\Aliyun_OSS_Media\Aws3\JmesPath\AstRuntime();
            case 'on':
                return new \GuztiaConsulting\Aliyun_OSS_Media\Aws3\JmesPath\CompilerRuntime();
            default:
                return new \GuztiaConsulting\Aliyun_OSS_Media\Aws3\JmesPath\CompilerRuntime($compileDir);
        }
    }
    /**
