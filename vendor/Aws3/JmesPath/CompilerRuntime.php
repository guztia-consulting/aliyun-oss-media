...
class CompilerRuntime
{
     ...
     */
    public function __construct($dir = null, \GuztiaConsulting\Aliyun_OSS_Media\Aws3\JmesPath\Parser $parser = null)
    {
        $this->parser = $parser ?: new \GuztiaConsulting\Aliyun_OSS_Media\Aws3\JmesPath\Parser();
        $this->compiler = new \GuztiaConsulting\Aliyun_OSS_Media\Aws3\JmesPath\TreeCompiler();
        $dir = $dir ?: sys_get_temp_dir();
        if (!is_dir($dir) && !mkdir($dir, 0755, true)) {
            throw new \RuntimeException("Unable to create cache directory: {$dir}");
        }
        $this->cacheDir = realpath($dir);
        $this->interpreter = new \GuztiaConsulting\Aliyun_OSS_Media\Aws3\JmesPath\TreeInterpreter();
    }
    /**
