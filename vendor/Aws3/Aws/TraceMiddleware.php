...
class TraceMiddleware
{
     ...
     *   "scrub_auth" is set to true.
     */
    public function __construct(array $config = [], \GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\Api\Service $service = null)
    {
    ...
    public function __invoke($step, $name)
    {
        ...
        return function (callable $next) use($step, $name) {
            return function (\GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\CommandInterface $command, \GuztiaConsulting\Aliyun_OSS_Media\Aws3\Psr\Http\Message\RequestInterface $request = null) use($next, $step, $name) {
    ...
    private function commandArray(\GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\CommandInterface $cmd)
    {
        return ['instance' => spl_object_hash($cmd), 'name' => $cmd->getName(), 'params' => $this->getRedactedArray($cmd)];
    }
    private function requestArray(\GuztiaConsulting\Aliyun_OSS_Media\Aws3\Psr\Http\Message\RequestInterface $request = null)
    {
    ...
    private function responseArray(\GuztiaConsulting\Aliyun_OSS_Media\Aws3\Psr\Http\Message\ResponseInterface $response = null)
    {
    ...
    private function streamStr(\GuztiaConsulting\Aliyun_OSS_Media\Aws3\Psr\Http\Message\StreamInterface $body)
    {
        return $body->getSize() < $this->config['stream_size'] ? (string) $body : 'stream(size=' . $body->getSize() . ')';
    }
    private function createHttpDebug(\GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\CommandInterface $command)
    {
        if ($this->config['http'] && !isset($command['@http']['debug'])) {
            $command['@http']['debug'] = fopen('php://temp', 'w+');
        }
    }
    private function flushHttpDebug(\GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\CommandInterface $command)
    {
     ...
     * @return array
     */
    private function getRedactedArray(\GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\CommandInterface $cmd)
    {
