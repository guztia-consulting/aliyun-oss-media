...
namespace GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\Sts;
 ...
 */
class StsClient extends \GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\AwsClient
{
    /**
     * {@inheritdoc}
     *
    ...
    public function __construct(array $args)
    {
        if (!isset($args['sts_regional_endpoints']) || $args['sts_regional_endpoints'] instanceof CacheInterface) {
            $args['sts_regional_endpoints'] = \GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\Sts\RegionalEndpoints\ConfigurationProvider::defaultProvider($args);
        }
     ...
     */
    public function createCredentials(\GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\Result $result)
    {
        if (!$result->hasKey('Credentials')) {
            throw new \InvalidArgumentException('Result contains no credentials');
        }
        $c = $result['Credentials'];
        return new \GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\Credentials\Credentials($c['AccessKeyId'], $c['SecretAccessKey'], isset($c['SessionToken']) ? $c['SessionToken'] : null, isset($c['Expiration']) && $c['Expiration'] instanceof \DateTimeInterface ? (int) $c['Expiration']->format('U') : null);
    }
}
