...
use GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\Kms\KmsClient;
/**
 ...
 */
class KmsMaterialsProviderV2 extends \GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\Crypto\MaterialsProviderV2 implements \GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\Crypto\MaterialsProviderInterfaceV2
{
     ...
     *                         and decrypting keys.
     */
    public function __construct(\GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\Kms\KmsClient $kmsClient, $kmsKeyId = null)
    {
    ...
    {
        if (empty($this->kmsKeyId)) {
            throw new \GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\Exception\CryptoException('A KMS key id is required for encryption' . ' with KMS keywrap. Use a KmsMaterialsProviderV2 that has been' . ' instantiated with a KMS key id.');
        }
        $options = array_change_key_case($options);
        if (!isset($options['@kmsencryptioncontext']) || !is_array($options['@kmsencryptioncontext'])) {
            throw new \GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\Exception\CryptoException("'@KmsEncryptionContext' is a" . " required argument when using KmsMaterialsProviderV2, and" . " must be an associative array (or empty array).");
        }
        if (isset($options['@kmsencryptioncontext']['aws:x-amz-cek-alg'])) {
            throw new \GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\Exception\CryptoException("Conflict in reserved @KmsEncryptionContext" . " key aws:x-amz-cek-alg. This value is reserved for the S3" . " Encryption Client and cannot be set by the user.");
        }
