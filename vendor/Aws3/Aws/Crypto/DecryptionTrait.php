...
trait DecryptionTrait
{
     ...
     * @internal
     */
    public function decrypt($cipherText, \GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\Crypto\MaterialsProviderInterface $provider, \GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\Crypto\MetadataEnvelope $envelope, array $cipherOptions = [])
    {
        $cipherOptions['Iv'] = base64_decode($envelope[\GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\Crypto\MetadataEnvelope::IV_HEADER]);
        $cipherOptions['TagLength'] = $envelope[\GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\Crypto\MetadataEnvelope::CRYPTO_TAG_LENGTH_HEADER] / 8;
        $cek = $provider->decryptCek(base64_decode($envelope[\GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\Crypto\MetadataEnvelope::CONTENT_KEY_V2_HEADER]), json_decode($envelope[\GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\Crypto\MetadataEnvelope::MATERIALS_DESCRIPTION_HEADER], true));
        $cipherOptions['KeySize'] = strlen($cek) * 8;
        $cipherOptions['Cipher'] = $this->getCipherFromAesName($envelope[\GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\Crypto\MetadataEnvelope::CONTENT_CRYPTO_SCHEME_HEADER]);
