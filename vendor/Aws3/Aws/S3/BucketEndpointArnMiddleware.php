...
use GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\Arn\ArnParser;
...
use GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\Arn\S3\OutpostsAccessPointArn;
...
use GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\S3\Exception\S3Exception;
...
class BucketEndpointArnMiddleware
{
     ...
     * @return callable
     */
    public static function wrap(\GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\Api\Service $service, $region, array $config)
    {
        return function (callable $handler) use($service, $region, $config) {
            return new self($handler, $service, $region, $config);
        };
    }
    public function __construct(callable $nextHandler, \GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\Api\Service $service, $region, array $config = [])
    {
        $this->partitionProvider = \GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\Endpoint\PartitionEndpointProvider::defaultProvider();
        $this->region = $region;
        $this->service = $service;
        $this->config = $config;
        $this->nextHandler = $nextHandler;
    }
    public function __invoke(\GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\CommandInterface $cmd, \GuztiaConsulting\Aliyun_OSS_Media\Aws3\Psr\Http\Message\RequestInterface $req)
    {
        ...
        if (!empty($op['input']['shape'])) {
            ...
            if (!empty($input = $service['shapes'][$op['input']['shape']])) {
                ...
                if (!empty($arnableKey) && \GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\Arn\ArnParser::isArn($cmd[$arnableKey])) {
                    try {
                        // Throw for commands that do not support ARN inputs
                        if (in_array($cmd->getName(), $this->nonArnableCommands)) {
                            throw new \GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\S3\Exception\S3Exception('ARN values cannot be used in the bucket field for' . ' the ' . $cmd->getName() . ' operation.', $cmd);
                        }
                        $arn = \GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\Arn\ArnParser::parse($cmd[$arnableKey]);
                        ...
                        throw new \GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\S3\Exception\S3Exception('Bucket parameter parsed as ARN and failed with: ' . $e->getMessage(), $cmd, [], $e);
                    }
                }
            }
        }
    ...
    private function generateAccessPointHost(\GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\Arn\AccessPointArn $arn, \GuztiaConsulting\Aliyun_OSS_Media\Aws3\Psr\Http\Message\RequestInterface $req)
    {
    ...
    private function validateArn($arn)
    {
        if ($arn instanceof AccessPointArnInterface) {
            ...
            if ($arn instanceof OutpostsAccessPointArn && !empty($this->config['dual_stack'])) {
                throw new \GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\Exception\UnresolvedEndpointException('Dualstack is currently not supported with S3 Outposts access' . ' points. Please disable dualstack or do not supply an' . ' access point ARN.');
            }
            // Accelerate is not supported with access points
            if (!empty($this->config['accelerate'])) {
                throw new \GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\Exception\UnresolvedEndpointException('Accelerate is currently not supported with access points.' . ' Please disable accelerate or do not supply an access' . ' point ARN.');
            }
            // Path-style is not supported with access points
            if (!empty($this->config['path_style'])) {
                throw new \GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\Exception\UnresolvedEndpointException('Path-style addressing is currently not supported with' . ' access points. Please disable path-style or do not' . ' supply an access point ARN.');
            }
            // Custom endpoint is not supported with access points
            if (!is_null($this->config['endpoint'])) {
                throw new \GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\Exception\UnresolvedEndpointException('A custom endpoint has been supplied along with an access' . ' point ARN, and these are not compatible with each other.' . ' Please only use one or the other.');
            }
            ...
            if ($arn->getPartition() !== $clientPart->getName()) {
                throw new \GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\Exception\InvalidRegionException('The supplied ARN partition' . " does not match the client's partition.");
            }
            if ($clientPart->getName() !== $arnPart->getName()) {
                throw new \GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\Exception\InvalidRegionException('The corresponding partition' . ' for the supplied ARN region does not match the' . " client's partition.");
            }
        ...
        throw new \GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\Arn\Exception\InvalidArnException('Provided ARN was not a valid S3 access' . ' point ARN or S3 Outposts access point ARN.');
    }
}
