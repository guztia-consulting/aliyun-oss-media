...
use GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\Multipart\AbstractUploadManager;
use GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\ResultInterface;
use GuztiaConsulting\Aliyun_OSS_Media\Aws3\GuzzleHttp\Psr7;
class MultipartCopy extends \GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\Multipart\AbstractUploadManager
{
    ...
    {
        if (\GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\Arn\ArnParser::isArn($source)) {
            $this->source = '';
        } else {
            $this->source = "/";
        }
    ...
    private function fetchSourceMetadata()
    {
        ...
        if (strpos($key, '?')) {
            ...
            $headParams['Key'] = $key;
            $query = \GuztiaConsulting\Aliyun_OSS_Media\Aws3\GuzzleHttp\Psr7\parse_query($query, false);
