...
use GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\MultiRegionClient as BaseClient;
 ...
 */
class S3MultiRegionClient extends \GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\MultiRegionClient implements \GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\S3\S3ClientInterface
{
    use S3ClientTrait;
    /** @var CacheInterface */
    private $cache;
    public static function getArguments()
    {
        ...
        unset($args['region']);
        return $args + ['bucket_region_cache' => ['type' => 'config', 'valid' => [\GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\CacheInterface::class], 'doc' => 'Cache of regions in which given buckets are located.', 'default' => function () {
            return new \GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\LruArrayCache();
        }], 'region' => $regionDef];
    }
    ...
    private function determineRegionMiddleware()
    {
        return function (callable $handler) {
            return function (\GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\CommandInterface $command) use($handler) {
                $cacheKey = $this->getCacheKey($command['Bucket']);
                if (empty($command['@region']) && ($region = $this->cache->get($cacheKey))) {
                    $command['@region'] = $region;
                }
                return \GuztiaConsulting\Aliyun_OSS_Media\Aws3\GuzzleHttp\Promise\coroutine(function () use($handler, $command, $cacheKey) {
                    try {
    ...
    public function determineBucketRegionAsync($bucketName)
    {
        ...
        if ($cached = $this->cache->get($cacheKey)) {
            return \GuztiaConsulting\Aliyun_OSS_Media\Aws3\GuzzleHttp\Promise\promise_for($cached);
        }
