...
class BucketEndpointMiddleware
{
    ...
    private function modifyRequest(\GuztiaConsulting\Aliyun_OSS_Media\Aws3\Psr\Http\Message\RequestInterface $request, \GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\CommandInterface $command)
    {
        $uri = $request->getUri();
        $path = $uri->getPath();
        $bucket = $command['Bucket'];
        ...
        if ($command['Key']) {
            $path = \GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\S3\S3Client::encodeKey(rawurldecode($path));
        }
