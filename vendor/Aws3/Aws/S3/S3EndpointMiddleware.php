...
class S3EndpointMiddleware
{
    ...
    public function __construct(callable $nextHandler, $region, array $options, $endpointProvider = null)
    {
        ...
        $this->endpointProvider = is_null($endpointProvider) ? \GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\Endpoint\PartitionEndpointProvider::defaultProvider() : $endpointProvider;
    ...
    private static function isRequestHostStyleCompatible(\GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\CommandInterface $command, \GuztiaConsulting\Aliyun_OSS_Media\Aws3\Psr\Http\Message\RequestInterface $request)
    {
        return \GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\S3\S3Client::isBucketDnsCompatible($command['Bucket']) && ($request->getUri()->getScheme() === 'http' || strpos($command['Bucket'], '.') === false) && filter_var($request->getUri()->getHost(), FILTER_VALIDATE_IP) === false;
    }
    private function endpointPatternDecider(\GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\CommandInterface $command, \GuztiaConsulting\Aliyun_OSS_Media\Aws3\Psr\Http\Message\RequestInterface $request)
    {
    ...
    private function canAccelerate(\GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\CommandInterface $command)
    {
        return empty(self::$exclusions[$command->getName()]) && \GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\S3\S3Client::isBucketDnsCompatible($command['Bucket']);
    }
    private function getBucketStyleHost(\GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\CommandInterface $command, $host)
    {
    ...
    private function applyHostStyleEndpoint(\GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\CommandInterface $command, \GuztiaConsulting\Aliyun_OSS_Media\Aws3\Psr\Http\Message\RequestInterface $request)
    {
        $uri = $request->getUri();
    ...
    private function applyDualStackEndpoint(\GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\CommandInterface $command, \GuztiaConsulting\Aliyun_OSS_Media\Aws3\Psr\Http\Message\RequestInterface $request)
    {
    ...
    private function applyAccelerateEndpoint(\GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\CommandInterface $command, \GuztiaConsulting\Aliyun_OSS_Media\Aws3\Psr\Http\Message\RequestInterface $request, $pattern)
    {
    ...
    private function getAccelerateHost(\GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\CommandInterface $command, $pattern)
    {
        $dnsSuffix = $this->endpointProvider->getPartition($this->region, 's3')->getDnsSuffix();
        return "{$command['Bucket']}.{$pattern}.{$dnsSuffix}";
    }
    private function getBucketlessPath($path, \GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\CommandInterface $command)
    {
