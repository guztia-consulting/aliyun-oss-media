...
class IdempotencyTokenMiddleware
{
     ...
     * @return callable
     */
    public static function wrap(\GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\Api\Service $service, callable $bytesGenerator = null)
    {
        return function (callable $handler) use($service, $bytesGenerator) {
            return new self($handler, $service, $bytesGenerator);
        };
    }
    public function __construct(callable $nextHandler, \GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\Api\Service $service, callable $bytesGenerator = null)
    {
