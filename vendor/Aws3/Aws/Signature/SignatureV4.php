...
use GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\Exception\CouldNotCreateChecksumException;
 ...
 */
class SignatureV4 implements \GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\Signature\SignatureInterface
{
    use SignatureTrait;
     ...
     */
    public static function convertPostToGet(\GuztiaConsulting\Aliyun_OSS_Media\Aws3\Psr\Http\Message\RequestInterface $request, $additionalQueryParams = "")
    {
        if ($request->getMethod() !== 'POST') {
            throw new \InvalidArgumentException('Expected a POST request but ' . 'received a ' . $request->getMethod() . ' request.');
        }
        $sr = $request->withMethod('GET')->withBody(\GuztiaConsulting\Aliyun_OSS_Media\Aws3\GuzzleHttp\Psr7\stream_for(''))->withoutHeader('Content-Type')->withoutHeader('Content-Length');
    ...
    protected function getPayload(\GuztiaConsulting\Aliyun_OSS_Media\Aws3\Psr\Http\Message\RequestInterface $request)
    {
        ...
        if (!$request->getBody()->isSeekable()) {
            throw new \GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\Exception\CouldNotCreateChecksumException('sha256');
        }
        try {
            return \GuztiaConsulting\Aliyun_OSS_Media\Aws3\GuzzleHttp\Psr7\hash($request->getBody(), 'sha256');
        } catch (\Exception $e) {
            throw new \GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\Exception\CouldNotCreateChecksumException('sha256', $e);
        }
    }
    ...
    private function createPresignedRequest(\GuztiaConsulting\Aliyun_OSS_Media\Aws3\Psr\Http\Message\RequestInterface $request, \GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\Credentials\CredentialsInterface $credentials)
    {
    ...
    private function parseRequest(\GuztiaConsulting\Aliyun_OSS_Media\Aws3\Psr\Http\Message\RequestInterface $request)
    {
        ...
        $uri = $request->getUri();
        return ['method' => $request->getMethod(), 'path' => $uri->getPath(), 'query' => \GuztiaConsulting\Aliyun_OSS_Media\Aws3\GuzzleHttp\Psr7\parse_query($uri->getQuery()), 'uri' => $uri, 'headers' => $request->getHeaders(), 'body' => $request->getBody(), 'version' => $request->getProtocolVersion()];
    }
    private function buildRequest(array $req)
    {
        if ($req['query']) {
            $req['uri'] = $req['uri']->withQuery(\GuztiaConsulting\Aliyun_OSS_Media\Aws3\GuzzleHttp\Psr7\build_query($req['query']));
        }
        return new \GuztiaConsulting\Aliyun_OSS_Media\Aws3\GuzzleHttp\Psr7\Request($req['method'], $req['uri'], $req['headers'], $req['body'], $req['version']);
    }
}
