...
use GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\ClientSideMonitoring\ConfigurationProvider;
...
use GuztiaConsulting\Aliyun_OSS_Media\Aws3\GuzzleHttp\Psr7\Uri;
/**
 * Default AWS client implementation
 */
class AwsClient implements \GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\AwsClientInterface
{
    use AwsClientTrait;
    /** @var array */
    private $aliases;
    ...
    {
        return \GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\ClientResolver::getDefaultArguments();
    }
    /**
    ...
    public function __construct(array $args)
    {
        ...
        $this->handlerList = new \GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\HandlerList();
        $resolver = new \GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\ClientResolver(static::getArguments());
        $config = $resolver->resolve($args, $this->handlerList);
        $this->api = $config['api'];
        $this->signatureProvider = $config['signature_provider'];
        $this->endpoint = new \GuztiaConsulting\Aliyun_OSS_Media\Aws3\GuzzleHttp\Psr7\Uri($config['endpoint']);
    ...
    public function getCommand($name, array $args = [])
    {
        ...
        return new \GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\Command($name, $args, clone $this->getHandlerList());
    }
    ...
    private function parseClass()
    {
        ...
        if ($klass === __CLASS__) {
            return ['', 'GuztiaConsulting\\Aliyun_OSS_Media\\Aws3\\Aws\\Exception\\AwsException'];
        }
        $service = substr($klass, strrpos($klass, '\\') + 1, -6);
        return [strtolower($service), "GuztiaConsulting\\Aliyun_OSS_Media\\Aws3\\Aws\\{$service}\\Exception\\{$service}Exception"];
    }
    private function addEndpointParameterMiddleware($args)
    {
        if (empty($args['disable_host_prefix_injection'])) {
            $list = $this->getHandlerList();
            $list->appendBuild(\GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\EndpointParameterMiddleware::wrap($this->api), 'endpoint_parameter');
        }
    }
    private function addEndpointDiscoveryMiddleware($config, $args)
    {
        ...
        if (!isset($args['endpoint'])) {
            $list->appendBuild(\GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\EndpointDiscovery\EndpointDiscoveryMiddleware::wrap($this, $args, $config['endpoint_discovery']), 'EndpointDiscoveryMiddleware');
        }
    }
    private function addSignatureMiddleware()
    {
        ...
        $resolver = static function (\GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\CommandInterface $c) use($api, $provider, $name, $region, $version) {
            ...
            return \GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\Signature\SignatureProvider::resolve($provider, $version, $name, $region);
        };
        $this->handlerList->appendSign(\GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\Middleware::signer($this->credentialProvider, $resolver), 'signer');
    }
    private function addInvocationId()
    {
        // Add invocation id to each request
        $this->handlerList->prependSign(\GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\Middleware::invocationId(), 'invocation-id');
    }
    private function loadAliases($file = null)
    {
        if (!isset($this->aliases)) {
            ...
            $aliases = \GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\load_compiled_json($file);
    ...
    {
        $streamRequestPayloadMiddleware = \GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\StreamRequestPayloadMiddleware::wrap($this->api);
    ...
    {
        $aliases = \GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\load_compiled_json(__DIR__ . '/data/aliases.json');
