...
namespace GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\EndpointDiscovery;
...
use GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\Exception\UnresolvedEndpointException;
use GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\LruArrayCache;
...
use GuztiaConsulting\Aliyun_OSS_Media\Aws3\Psr\Http\Message\UriInterface;
class EndpointDiscoveryMiddleware
{
    ...
    public function __construct(callable $handler, \GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\AwsClient $client, array $args, $config)
    {
    ...
    public function __invoke(\GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\CommandInterface $cmd, \GuztiaConsulting\Aliyun_OSS_Media\Aws3\Psr\Http\Message\RequestInterface $request)
    {
        ...
        if (isset($op['endpointdiscovery'])) {
            $config = \GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\EndpointDiscovery\ConfigurationProvider::unwrap($this->config);
            $isRequired = !empty($op['endpointdiscovery']['required']);
            if ($isRequired && !$config->isEnabled()) {
                throw new \GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\Exception\UnresolvedEndpointException('This operation ' . 'requires the use of endpoint discovery, but this has ' . 'been disabled in the configuration. Enable endpoint ' . 'discovery or use a different operation.');
            }
            // Continue only if enabled by config
            if ($config->isEnabled()) {
                if (isset($op['endpointoperation'])) {
                    throw new \GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\Exception\UnresolvedEndpointException('This operation is ' . 'contradictorily marked both as using endpoint discovery ' . 'and being the endpoint discovery operation. Please ' . 'verify the accuracy of your model files.');
                }
                ...
                // Check/create cache
                if (!isset(self::$cache)) {
                    self::$cache = new \GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\LruArrayCache($config->getCacheLimit());
                }
                if (empty($endpointList = self::$cache->get($cacheKey))) {
                    $endpointList = new \GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\EndpointDiscovery\EndpointList([]);
                }
    ...
    private function discoverEndpoint($cacheKey, \GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\CommandInterface $cmd, array $identifiers)
    {
        ...
        if (isset($result['Endpoints'])) {
            ...
            $endpointList = new \GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\EndpointDiscovery\EndpointList($endpointData);
            self::$cache->set($cacheKey, $endpointList);
            return $endpointList->getEndpoint();
        }
        throw new \GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\Exception\UnresolvedEndpointException('The endpoint discovery operation ' . 'yielded a response that did not contain properly formatted ' . 'endpoint data.');
    }
    private function getCacheKey(\GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\Credentials\CredentialsInterface $creds, \GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\CommandInterface $cmd, array $identifiers)
    {
    ...
    private function getDiscoveryCommand(\GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\CommandInterface $cmd, array $identifiers)
    {
        ...
        if (!isset($endpointOperation)) {
            throw new \GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\Exception\UnresolvedEndpointException('This command is set to use ' . 'endpoint discovery, but no endpoint discovery operation was ' . 'found. Please verify the accuracy of your model files.');
        }
        $params = [];
        if (!empty($identifiers)) {
        ...
        $command->getHandlerList()->appendBuild(\GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\Middleware::mapRequest(function (\GuztiaConsulting\Aliyun_OSS_Media\Aws3\Psr\Http\Message\RequestInterface $r) {
    ...
    private function handleDiscoveryException($isRequired, $originalUri, \Exception $e, \GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\CommandInterface $cmd, \GuztiaConsulting\Aliyun_OSS_Media\Aws3\Psr\Http\Message\RequestInterface $request)
    {
        ...
        if ($isRequired) {
            ...
            throw new \GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\Exception\AwsException($message, $cmd, ['code' => 'EndpointDiscoveryException', 'message' => $message], $e);
        }
    ...
    private function modifyRequest(\GuztiaConsulting\Aliyun_OSS_Media\Aws3\Psr\Http\Message\RequestInterface $request, $endpoint)
    {
    ...
    private function parseEndpoint($endpoint)
    {
        ...
        throw new \GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\Exception\UnresolvedEndpointException("The supplied endpoint '" . "{$endpoint}' is invalid.");
    }
    private function useOriginalUri(\GuztiaConsulting\Aliyun_OSS_Media\Aws3\Psr\Http\Message\UriInterface $uri, \GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\CommandInterface $cmd, \GuztiaConsulting\Aliyun_OSS_Media\Aws3\Psr\Http\Message\RequestInterface $request)
    {
        $nextHandler = $this->nextHandler;
