...
class WrappedHttpHandler
{
     ...
     *                                 information.
     */
    public function __construct(callable $httpHandler, callable $parser, callable $errorParser, $exceptionClass = 'GuztiaConsulting\\Aliyun_OSS_Media\\Aws3\\Aws\\Exception\\AwsException', $collectStats = false)
    {
    ...
    public function __invoke(\GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\CommandInterface $command, \GuztiaConsulting\Aliyun_OSS_Media\Aws3\Psr\Http\Message\RequestInterface $request)
    {
        ...
        return \GuztiaConsulting\Aliyun_OSS_Media\Aws3\GuzzleHttp\Promise\promise_for($fn($request, $options))->then(function (\GuztiaConsulting\Aliyun_OSS_Media\Aws3\Psr\Http\Message\ResponseInterface $res) use($command, $request, &$stats) {
            ...
            return new \GuztiaConsulting\Aliyun_OSS_Media\Aws3\GuzzleHttp\Promise\RejectedPromise($err);
        });
    }
    /**
     ...
     */
    private function parseResponse(\GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\CommandInterface $command, \GuztiaConsulting\Aliyun_OSS_Media\Aws3\Psr\Http\Message\RequestInterface $request, \GuztiaConsulting\Aliyun_OSS_Media\Aws3\Psr\Http\Message\ResponseInterface $response, array $stats)
    {
        $parser = $this->parser;
        $status = $response->getStatusCode();
        $result = $status < 300 ? $parser($command, $response) : new \GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\Result();
     ...
     * @return \Exception
     */
    private function parseError(array $err, \GuztiaConsulting\Aliyun_OSS_Media\Aws3\Psr\Http\Message\RequestInterface $request, \GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\CommandInterface $command, array $stats)
    {
