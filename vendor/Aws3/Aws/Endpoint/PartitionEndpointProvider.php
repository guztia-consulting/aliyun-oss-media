...
use GuztiaConsulting\Aliyun_OSS_Media\Aws3\JmesPath\Env;
class PartitionEndpointProvider
{
    ...
    public function __construct(array $partitions, $defaultPartition = 'aws', $options = [])
    {
        $this->partitions = array_map(function (array $definition) {
            return new \GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\Endpoint\Partition($definition);
    ...
    {
        $data = \GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\load_compiled_json(__DIR__ . '/../data/endpoints.json');
        $prefixData = \GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\load_compiled_json(__DIR__ . '/../data/endpoints_prefix_history.json');
    ...
    public static function mergePrefixData($data, $prefixData)
    {
        ...
        foreach ($data["partitions"] as $index => $partition) {
            foreach ($prefixGroups as $current => $old) {
                $serviceData = \GuztiaConsulting\Aliyun_OSS_Media\Aws3\JmesPath\Env::search("services.\"{$current}\"", $partition);
                if (!empty($serviceData)) {
                    foreach ($old as $prefix) {
                        if (empty(\GuztiaConsulting\Aliyun_OSS_Media\Aws3\JmesPath\Env::search("services.\"{$prefix}\"", $partition))) {
