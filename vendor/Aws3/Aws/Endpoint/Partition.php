...
namespace GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\Endpoint;

use ArrayAccess;
use GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\HasDataTrait;
use GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\Sts\RegionalEndpoints\ConfigurationProvider;
use GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\S3\RegionalEndpoint\ConfigurationProvider as S3ConfigurationProvider;
use InvalidArgumentException as Iae;
/**
 * Default implementation of an AWS partition.
 */
final class Partition implements \ArrayAccess, \GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\Endpoint\PartitionInterface
{
    use HasDataTrait;
    ...
    {
        return $service === 'sts' && in_array($region, $this->stsLegacyGlobalRegions) && (empty($options['sts_regional_endpoints']) || \GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\Sts\RegionalEndpoints\ConfigurationProvider::unwrap($options['sts_regional_endpoints'])->getEndpointsType() !== 'regional');
    }
    /**
    ...
    {
        return $service === 's3' && $region === 'us-east-1' && (empty($options['s3_us_east_1_regional_endpoint']) || \GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\S3\RegionalEndpoint\ConfigurationProvider::unwrap($options['s3_us_east_1_regional_endpoint'])->getEndpointsType() !== 'regional');
    }
