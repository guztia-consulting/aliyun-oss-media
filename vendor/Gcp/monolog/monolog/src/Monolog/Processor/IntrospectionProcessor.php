 ...
 */
class IntrospectionProcessor implements \GuztiaConsulting\Aliyun_OSS_Media\Gcp\Monolog\Processor\ProcessorInterface
{
    private $level;
     ...
     */
    public function __construct($level = \GuztiaConsulting\Aliyun_OSS_Media\Gcp\Monolog\Logger::DEBUG, array $skipClassesPartials = [], int $skipStackFramesCount = 0)
    {
        $this->level = \GuztiaConsulting\Aliyun_OSS_Media\Gcp\Monolog\Logger::toMonologLevel($level);
        $this->skipClassesPartials = array_merge(['GuztiaConsulting\\Aliyun_OSS_Media\\Gcp\\Monolog\\'], $skipClassesPartials);
