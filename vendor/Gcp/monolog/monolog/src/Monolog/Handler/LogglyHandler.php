...
use GuztiaConsulting\Aliyun_OSS_Media\Gcp\Monolog\Formatter\LogglyFormatter;
use function GuztiaConsulting\Aliyun_OSS_Media\Gcp\array_key_exists;
/**
 ...
 */
class LogglyHandler extends \GuztiaConsulting\Aliyun_OSS_Media\Gcp\Monolog\Handler\AbstractProcessingHandler
{
     ...
     */
    public function __construct(string $token, $level = \GuztiaConsulting\Aliyun_OSS_Media\Gcp\Monolog\Logger::DEBUG, bool $bubble = true)
    {
        if (!extension_loaded('curl')) {
            throw new \GuztiaConsulting\Aliyun_OSS_Media\Gcp\Monolog\Handler\MissingExtensionException('The curl extension is needed to use the LogglyHandler');
        }
        $this->token = $token;
    ...
    {
        if (!\GuztiaConsulting\Aliyun_OSS_Media\Gcp\array_key_exists($endpoint, $this->curlHandlers)) {
    ...
    protected function send(string $data, string $endpoint) : void
    {
        ...
        \GuztiaConsulting\Aliyun_OSS_Media\Gcp\Monolog\Handler\Curl\Util::execute($ch, 5, false);
    }
    protected function getDefaultFormatter() : FormatterInterface
    {
        return new \GuztiaConsulting\Aliyun_OSS_Media\Gcp\Monolog\Formatter\LogglyFormatter();
    }
}
