 ...
 */
class BrowserConsoleHandler extends \GuztiaConsulting\Aliyun_OSS_Media\Gcp\Monolog\Handler\AbstractProcessingHandler
{
    ...
    {
        return new \GuztiaConsulting\Aliyun_OSS_Media\Gcp\Monolog\Formatter\LineFormatter('[[%channel%]]{macro: autolabel} [[%level_name%]]{font-weight: bold} %message%');
    }
    /**
     * {@inheritDoc}
     */
    ...
    {
        if (PHP_SAPI !== 'cli') {
            register_shutdown_function(['GuztiaConsulting\\Aliyun_OSS_Media\\Gcp\\Monolog\\Handler\\BrowserConsoleHandler', 'send']);
        }
    }
    /**
    ...
    private static function handleStyles(string $formatted) : array
    {
        ...
        foreach (array_reverse($matches) as $match) {
            ...
            $pos = $match[0][1];
            $format = \GuztiaConsulting\Aliyun_OSS_Media\Gcp\Monolog\Utils::substr($format, 0, $pos) . '%c' . $match[1][0] . '%c' . \GuztiaConsulting\Aliyun_OSS_Media\Gcp\Monolog\Utils::substr($format, $pos + strlen($match[0][0]));
        }
