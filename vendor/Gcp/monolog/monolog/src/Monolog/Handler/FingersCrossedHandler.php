...
use GuztiaConsulting\Aliyun_OSS_Media\Gcp\Monolog\Handler\FingersCrossed\ErrorLevelActivationStrategy;
use GuztiaConsulting\Aliyun_OSS_Media\Gcp\Monolog\Handler\FingersCrossed\ActivationStrategyInterface;
...
class FingersCrossedHandler extends \GuztiaConsulting\Aliyun_OSS_Media\Gcp\Monolog\Handler\Handler implements \GuztiaConsulting\Aliyun_OSS_Media\Gcp\Monolog\Handler\ProcessableHandlerInterface, \GuztiaConsulting\Aliyun_OSS_Media\Gcp\Monolog\ResettableInterface, \GuztiaConsulting\Aliyun_OSS_Media\Gcp\Monolog\Handler\FormattableHandlerInterface
{
    ...
    public function __construct($handler, $activationStrategy = null, int $bufferSize = 0, bool $bubble = true, bool $stopBuffering = true, $passthruLevel = null)
    {
        if (null === $activationStrategy) {
            $activationStrategy = new \GuztiaConsulting\Aliyun_OSS_Media\Gcp\Monolog\Handler\FingersCrossed\ErrorLevelActivationStrategy(\GuztiaConsulting\Aliyun_OSS_Media\Gcp\Monolog\Logger::WARNING);
        }
        // convert simple int activationStrategy to an object
        if (!$activationStrategy instanceof ActivationStrategyInterface) {
            $activationStrategy = new \GuztiaConsulting\Aliyun_OSS_Media\Gcp\Monolog\Handler\FingersCrossed\ErrorLevelActivationStrategy($activationStrategy);
        }
        ...
        if ($passthruLevel !== null) {
            $this->passthruLevel = \GuztiaConsulting\Aliyun_OSS_Media\Gcp\Monolog\Logger::toMonologLevel($passthruLevel);
        }
