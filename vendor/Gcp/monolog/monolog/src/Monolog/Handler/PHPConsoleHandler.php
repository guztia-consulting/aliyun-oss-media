...
use GuztiaConsulting\Aliyun_OSS_Media\Gcp\Monolog\Formatter\LineFormatter;
...
use GuztiaConsulting\Aliyun_OSS_Media\Gcp\PhpConsole\Connector;
use GuztiaConsulting\Aliyun_OSS_Media\Gcp\PhpConsole\Handler as VendorPhpConsoleHandler;
use GuztiaConsulting\Aliyun_OSS_Media\Gcp\PhpConsole\Helper;
/**
 ...
 */
class PHPConsoleHandler extends \GuztiaConsulting\Aliyun_OSS_Media\Gcp\Monolog\Handler\AbstractProcessingHandler
{
    private $options = [
        'enabled' => true,
        // bool Is PHP Console server enabled
        'classesPartialsTraceIgnore' => ['GuztiaConsulting\\Aliyun_OSS_Media\\Gcp\\Monolog\\'],
     ...
     */
    public function __construct(array $options = [], ?Connector $connector = null, $level = \GuztiaConsulting\Aliyun_OSS_Media\Gcp\Monolog\Logger::DEBUG, bool $bubble = true)
    {
        if (!class_exists('GuztiaConsulting\\Aliyun_OSS_Media\\Gcp\\PhpConsole\\Connector')) {
    ...
    private function initConnector(?Connector $connector = null) : Connector
    {
        if (!$connector) {
            if ($this->options['dataStorage']) {
                \GuztiaConsulting\Aliyun_OSS_Media\Gcp\PhpConsole\Connector::setPostponeStorage($this->options['dataStorage']);
            }
            $connector = \GuztiaConsulting\Aliyun_OSS_Media\Gcp\PhpConsole\Connector::getInstance();
        }
        if ($this->options['registerHelper'] && !\GuztiaConsulting\Aliyun_OSS_Media\Gcp\PhpConsole\Helper::isRegistered()) {
            \GuztiaConsulting\Aliyun_OSS_Media\Gcp\PhpConsole\Helper::register();
        }
        if ($this->options['enabled'] && $connector->isActiveClient()) {
            if ($this->options['useOwnErrorsHandler'] || $this->options['useOwnExceptionsHandler']) {
                $handler = \GuztiaConsulting\Aliyun_OSS_Media\Gcp\PhpConsole\Handler::getInstance();
    ...
    {
        if ($record['level'] < \GuztiaConsulting\Aliyun_OSS_Media\Gcp\Monolog\Logger::NOTICE) {
    ...
    private function handleDebugRecord(array $record) : void
    {
        ...
        if ($record['context']) {
            $message .= ' ' . \GuztiaConsulting\Aliyun_OSS_Media\Gcp\Monolog\Utils::jsonEncode($this->connector->getDumper()->dump(array_filter($record['context'])), null, true);
        }
    ...
    {
        return new \GuztiaConsulting\Aliyun_OSS_Media\Gcp\Monolog\Formatter\LineFormatter('%message%');
    }
}
