...
use GuztiaConsulting\Aliyun_OSS_Media\Gcp\Monolog\Handler\Slack\SlackRecord;
/**
 ...
 */
class SlackHandler extends \GuztiaConsulting\Aliyun_OSS_Media\Gcp\Monolog\Handler\SocketHandler
{
    /**
     * Slack API token
     * @var string
     ...
     */
    public function __construct(string $token, string $channel, ?string $username = null, bool $useAttachment = true, ?string $iconEmoji = null, $level = \GuztiaConsulting\Aliyun_OSS_Media\Gcp\Monolog\Logger::CRITICAL, bool $bubble = true, bool $useShortAttachment = false, bool $includeContextAndExtra = false, array $excludeFields = array())
    {
        if (!extension_loaded('openssl')) {
            throw new \GuztiaConsulting\Aliyun_OSS_Media\Gcp\Monolog\Handler\MissingExtensionException('The OpenSSL PHP extension is required to use the SlackHandler');
        }
        parent::__construct('ssl://slack.com:443', $level, $bubble);
        $this->slackRecord = new \GuztiaConsulting\Aliyun_OSS_Media\Gcp\Monolog\Handler\Slack\SlackRecord($channel, $username, $useAttachment, $iconEmoji, $useShortAttachment, $includeContextAndExtra, $excludeFields);
        $this->token = $token;
    }
    ...
    protected function prepareContentData(array $record) : array
    {
        ...
        if (!empty($dataArray['attachments'])) {
            $dataArray['attachments'] = \GuztiaConsulting\Aliyun_OSS_Media\Gcp\Monolog\Utils::jsonEncode($dataArray['attachments']);
        }
        return $dataArray;
    }
    /**
