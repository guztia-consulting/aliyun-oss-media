...
use GuztiaConsulting\Aliyun_OSS_Media\Gcp\MongoDB\BSON\UTCDateTime;
 ...
 */
class MongoDBFormatter implements \GuztiaConsulting\Aliyun_OSS_Media\Gcp\Monolog\Formatter\FormatterInterface
{
    ...
    protected function formatObject($value, int $nestingLevel)
    {
        ...
        $objectVars['class'] = \GuztiaConsulting\Aliyun_OSS_Media\Gcp\Monolog\Utils::getClass($value);
        return $this->formatArray($objectVars, $nestingLevel);
    }
    protected function formatException(\Throwable $exception, int $nestingLevel)
    {
        $formattedException = ['class' => \GuztiaConsulting\Aliyun_OSS_Media\Gcp\Monolog\Utils::getClass($exception), 'message' => $exception->getMessage(), 'code' => (int) $exception->getCode(), 'file' => $exception->getFile() . ':' . $exception->getLine()];
    ...
    {
        return new \GuztiaConsulting\Aliyun_OSS_Media\Gcp\MongoDB\BSON\UTCDateTime((int) (string) floor($value->format('U.u') * 1000));
    }
    /**
    ...
    private function legacyGetMongoDbDateTime(\DateTimeInterface $value) : UTCDateTime
    {
        ...
        return new \GuztiaConsulting\Aliyun_OSS_Media\Gcp\MongoDB\BSON\UTCDateTime($milliseconds);
    }
}
