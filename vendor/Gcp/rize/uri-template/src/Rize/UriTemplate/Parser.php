...
namespace GuztiaConsulting\Aliyun_OSS_Media\Gcp\Rize\UriTemplate;

use GuztiaConsulting\Aliyun_OSS_Media\Gcp\Rize\UriTemplate\Node;
use GuztiaConsulting\Aliyun_OSS_Media\Gcp\Rize\UriTemplate\Node\Expression;
use GuztiaConsulting\Aliyun_OSS_Media\Gcp\Rize\UriTemplate\Operator;
use GuztiaConsulting\Aliyun_OSS_Media\Gcp\Rize\UriTemplate\Operator\UnNamed;
class Parser
{
    ...
    protected function parseExpression($expression)
    {
        ...
        if (!\GuztiaConsulting\Aliyun_OSS_Media\Gcp\Rize\UriTemplate\Operator\Abstraction::isValid($prefix)) {
            // not valid chars?
    ...
    {
        return new \GuztiaConsulting\Aliyun_OSS_Media\Gcp\Rize\UriTemplate\Node\Variable($token, $options);
    }
    protected function createExpressionNode($token, \GuztiaConsulting\Aliyun_OSS_Media\Gcp\Rize\UriTemplate\Operator\Abstraction $operator = null, array $vars = array())
    {
        return new \GuztiaConsulting\Aliyun_OSS_Media\Gcp\Rize\UriTemplate\Node\Expression($token, $operator, $vars);
    }
    protected function createLiteralNode($token)
    {
        return new \GuztiaConsulting\Aliyun_OSS_Media\Gcp\Rize\UriTemplate\Node\Literal($token);
    }
    protected function createOperatorNode($token)
    {
        return \GuztiaConsulting\Aliyun_OSS_Media\Gcp\Rize\UriTemplate\Operator\Abstraction::createById($token);
    }
}
