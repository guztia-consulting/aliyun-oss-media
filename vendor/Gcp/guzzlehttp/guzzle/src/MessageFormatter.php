 ...
 * @final
 */
class MessageFormatter implements \GuztiaConsulting\Aliyun_OSS_Media\Gcp\GuzzleHttp\MessageFormatterInterface
{
    /**
    ...
    public function format(\GuztiaConsulting\Aliyun_OSS_Media\Gcp\Psr\Http\Message\RequestInterface $request, ?ResponseInterface $response = null, ?\Throwable $error = null) : string
    {
        ...
        return \preg_replace_callback('/{\\s*([A-Za-z_\\-\\.0-9]+)\\s*}/', function (array $matches) use($request, $response, $error, &$cache) {
            ...
            switch ($matches[1]) {
                case 'request':
                    $result = \GuztiaConsulting\Aliyun_OSS_Media\Gcp\GuzzleHttp\Psr7\Message::toString($request);
                    break;
                case 'response':
                    $result = $response ? \GuztiaConsulting\Aliyun_OSS_Media\Gcp\GuzzleHttp\Psr7\Message::toString($response) : '';
                    break;
                case 'req_headers':
     ...
     */
    private function headers(\GuztiaConsulting\Aliyun_OSS_Media\Gcp\Psr\Http\Message\MessageInterface $message) : string
    {
        $result = '';
