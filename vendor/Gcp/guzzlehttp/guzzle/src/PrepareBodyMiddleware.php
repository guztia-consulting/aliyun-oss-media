    ...
    public function __invoke(\GuztiaConsulting\Aliyun_OSS_Media\Gcp\Psr\Http\Message\RequestInterface $request, array $options) : PromiseInterface
    {
        ...
        if (!$request->hasHeader('Content-Type')) {
            if ($uri = $request->getBody()->getMetadata('uri')) {
                if (is_string($uri) && ($type = \GuztiaConsulting\Aliyun_OSS_Media\Gcp\GuzzleHttp\Psr7\MimeType::fromFilename($uri))) {
        ...
        return $fn(\GuztiaConsulting\Aliyun_OSS_Media\Gcp\GuzzleHttp\Psr7\Utils::modifyRequest($request, $modify), $options);
    }
    /**
     * Add expect header
     */
    private function addExpectHeader(\GuztiaConsulting\Aliyun_OSS_Media\Gcp\Psr\Http\Message\RequestInterface $request, array $options, array &$modify) : void
    {
