 ...
 */
class EachPromise implements \GuztiaConsulting\Aliyun_OSS_Media\Gcp\GuzzleHttp\Promise\PromisorInterface
{
    private $pending = [];
    /** @var \Iterator|null */
    private $iterable;
    ...
    {
        $this->iterable = \GuztiaConsulting\Aliyun_OSS_Media\Gcp\GuzzleHttp\Promise\Create::iterFor($iterable);
    ...
    {
        $this->mutex = false;
        $this->aggregate = new \GuztiaConsulting\Aliyun_OSS_Media\Gcp\GuzzleHttp\Promise\Promise(function () {
            reset($this->pending);
            ...
            while ($promise = current($this->pending)) {
                next($this->pending);
                $promise->wait();
                if (\GuztiaConsulting\Aliyun_OSS_Media\Gcp\GuzzleHttp\Promise\Is::settled($this->aggregate)) {
                    return;
                }
            }
        });
    ...
    private function addPending()
    {
        ...
        $promise = \GuztiaConsulting\Aliyun_OSS_Media\Gcp\GuzzleHttp\Promise\Create::promiseFor($this->iterable->current());
