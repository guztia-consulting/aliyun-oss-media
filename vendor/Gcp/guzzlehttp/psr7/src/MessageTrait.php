...
trait MessageTrait
{
    ...
    {
        if (!$this->stream) {
            $this->stream = \GuztiaConsulting\Aliyun_OSS_Media\Gcp\GuzzleHttp\Psr7\Utils::streamFor('');
        }
        return $this->stream;
    }
    public function withBody(\GuztiaConsulting\Aliyun_OSS_Media\Gcp\Psr\Http\Message\StreamInterface $body)
    {
        if ($body === $this->stream) {
            return $this;
        }
