...
trait SimpleJobTrait
{
    ...
    private function setSimpleJobProperties(array $options = [])
    {
        ...
        $config->registerJob($identifier, function ($id) use($identifier, $options) {
            return new \GuztiaConsulting\Aliyun_OSS_Media\Gcp\Google\Cloud\Core\Batch\SimpleJob($identifier, [$this, 'run'], $id, $options);
        });
        try {
    ...
    private function defaultConfigStorage()
    {
        if ($this->isSysvIPCLoaded() && $this->isDaemonRunning()) {
            return new \GuztiaConsulting\Aliyun_OSS_Media\Gcp\Google\Cloud\Core\Batch\SysvConfigStorage();
        } else {
            return \GuztiaConsulting\Aliyun_OSS_Media\Gcp\Google\Cloud\Core\Batch\InMemoryConfigStorage::getInstance();
        }
    }
}
