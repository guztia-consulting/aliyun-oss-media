 ...
 */
namespace GuztiaConsulting\Aliyun_OSS_Media\Gcp\Google\Cloud\Core\Batch;

use GuztiaConsulting\Aliyun_OSS_Media\Gcp\Google\Cloud\Core\SysvTrait;
/**
...
class BatchRunner
{
     ...
     */
    public function __construct(\GuztiaConsulting\Aliyun_OSS_Media\Gcp\Google\Cloud\Core\Batch\ConfigStorageInterface $configStorage = null, \GuztiaConsulting\Aliyun_OSS_Media\Gcp\Google\Cloud\Core\Batch\ProcessItemInterface $processor = null)
    {
        if ($configStorage === null || $processor === null) {
            if ($this->isSysvIPCLoaded() && $this->isDaemonRunning()) {
                $configStorage = new \GuztiaConsulting\Aliyun_OSS_Media\Gcp\Google\Cloud\Core\Batch\SysvConfigStorage();
                $processor = new \GuztiaConsulting\Aliyun_OSS_Media\Gcp\Google\Cloud\Core\Batch\SysvProcessor();
            } else {
                $configStorage = \GuztiaConsulting\Aliyun_OSS_Media\Gcp\Google\Cloud\Core\Batch\InMemoryConfigStorage::getInstance();
    ...
    public function registerJob($identifier, $func, array $options = [])
    {
        ...
        $this->config->registerJob($identifier, function ($id) use($identifier, $func, $options) {
            return new \GuztiaConsulting\Aliyun_OSS_Media\Gcp\Google\Cloud\Core\Batch\BatchJob($identifier, $func, $id, $options);
        });
        try {
