...
use GuztiaConsulting\Aliyun_OSS_Media\Gcp\Google\Cloud\Core\Compute\Metadata;
...
trait ClientTrait
{
    ...
    private function getConnectionType(array $config)
    {
        ...
        if ($transport === 'grpc') {
            if (!$isGrpcExtensionLoaded) {
                throw new \GuztiaConsulting\Aliyun_OSS_Media\Gcp\Google\Cloud\Core\Exception\GoogleException('gRPC support has been requested but required dependencies ' . 'have not been found. ' . $this->getGrpcInstallationMessage());
            }
        }
        return $transport;
    }
    ...
    private function requireGrpc()
    {
        if (!$this->isGrpcLoaded()) {
            throw new \GuztiaConsulting\Aliyun_OSS_Media\Gcp\Google\Cloud\Core\Exception\GoogleException('The requested client requires the gRPC extension. ' . $this->getGrpcInstallationMessage());
        }
    }
    /**
     * @return string
    ...
    private function getKeyFile(array $config = [])
    {
        ...
        if ($config['keyFilePath']) {
            if (!file_exists($config['keyFilePath'])) {
                throw new \GuztiaConsulting\Aliyun_OSS_Media\Gcp\Google\Cloud\Core\Exception\GoogleException(sprintf('Given keyfile path %s does not exist', $config['keyFilePath']));
            }
            try {
                $keyFileData = $this->jsonDecode(file_get_contents($config['keyFilePath']), true);
            } catch (\InvalidArgumentException $ex) {
                throw new \GuztiaConsulting\Aliyun_OSS_Media\Gcp\Google\Cloud\Core\Exception\GoogleException(sprintf('Given keyfile at path %s was invalid', $config['keyFilePath']));
            }
            return $keyFileData;
        }
        return \GuztiaConsulting\Aliyun_OSS_Media\Gcp\Google\Auth\CredentialsLoader::fromEnv() ?: \GuztiaConsulting\Aliyun_OSS_Media\Gcp\Google\Auth\CredentialsLoader::fromWellKnownFile();
    }
    /**
    ...
    {
        return \GuztiaConsulting\Aliyun_OSS_Media\Gcp\Google\Auth\Credentials\GCECredentials::onGce($httpHandler);
    }
    /**
    ...
    {
        return new \GuztiaConsulting\Aliyun_OSS_Media\Gcp\Google\Cloud\Core\Compute\Metadata();
    }
    /**
