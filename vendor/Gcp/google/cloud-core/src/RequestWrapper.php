...
use GuztiaConsulting\Aliyun_OSS_Media\Gcp\Google\Cloud\Core\RequestWrapperTrait;
...
class RequestWrapper
{
    ...
    public function __construct(array $config = [])
    {
        ...
        $this->httpHandler = $config['httpHandler'] ?: \GuztiaConsulting\Aliyun_OSS_Media\Gcp\Google\Auth\HttpHandler\HttpHandlerFactory::build();
     ...
     */
    public function send(\GuztiaConsulting\Aliyun_OSS_Media\Gcp\Psr\Http\Message\RequestInterface $request, array $options = [])
    {
        $retryOptions = $this->getRetryOptions($options);
        $backoff = new \GuztiaConsulting\Aliyun_OSS_Media\Gcp\Google\Cloud\Core\ExponentialBackoff($retryOptions['retries'], $retryOptions['retryFunction']);
     ...
     *      upgrading.
     */
    public function sendAsync(\GuztiaConsulting\Aliyun_OSS_Media\Gcp\Psr\Http\Message\RequestInterface $request, array $options = [])
    {
        ...
        $fn = function ($retryAttempt) use(&$fn, $request, $options) {
            ...
            $retryOptions = $this->getRetryOptions($options);
            if (!$retryOptions['calcDelayFunction']) {
                $retryOptions['calcDelayFunction'] = [\GuztiaConsulting\Aliyun_OSS_Media\Gcp\Google\Cloud\Core\ExponentialBackoff::class, 'calculateDelay'];
            }
     ...
     */
    private function applyHeaders(\GuztiaConsulting\Aliyun_OSS_Media\Gcp\Psr\Http\Message\RequestInterface $request)
    {
        ...
        return \GuztiaConsulting\Aliyun_OSS_Media\Gcp\GuzzleHttp\Psr7\modify_request($request, ['set_headers' => $headers]);
    }
    /**
     * Fetches credentials.
     *
     ...
     * @return array
     */
    private function fetchCredentials(\GuztiaConsulting\Aliyun_OSS_Media\Gcp\Google\Auth\FetchAuthTokenInterface $credentialsFetcher)
    {
        $backoff = new \GuztiaConsulting\Aliyun_OSS_Media\Gcp\Google\Cloud\Core\ExponentialBackoff($this->retries, $this->getRetryFunction());
        try {
    ...
    private function buildDefaultAsyncHandler()
    {
        ...
        return $isGuzzleHandler ? [$this->httpHandler, 'async'] : [\GuztiaConsulting\Aliyun_OSS_Media\Gcp\Google\Auth\HttpHandler\HttpHandlerFactory::build(), 'async'];
    }
}
