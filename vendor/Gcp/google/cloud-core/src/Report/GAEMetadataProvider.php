 ...
 */
namespace GuztiaConsulting\Aliyun_OSS_Media\Gcp\Google\Cloud\Core\Report;

/**
 * MetadataProvider for GAE.
 */
abstract class GAEMetadataProvider implements \GuztiaConsulting\Aliyun_OSS_Media\Gcp\Google\Cloud\Core\Report\MetadataProviderInterface
{
    ...
    {
        $projectId = isset($server['GOOGLE_CLOUD_PROJECT']) ? $server['GOOGLE_CLOUD_PROJECT'] : 'unknown-projectid';
        $serviceId = isset($server['GAE_SERVICE']) ? $server['GAE_SERVICE'] : 'unknown-service';
        $versionId = isset($server['GAE_VERSION']) ? $server['GAE_VERSION'] : 'unknown-version';
