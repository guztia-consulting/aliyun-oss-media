...
use GuztiaConsulting\Aliyun_OSS_Media\Gcp\GuzzleHttp\Psr7\Uri;
...
class RequestBuilder
{
    ...
    public function __construct($servicePath, $baseUri, array $resourceRoot = [])
    {
        ...
        if (isset($this->service['basePath'])) {
            ...
            if (!$uriParts['path'] || $uriParts['path'] === '/') {
                ...
                $this->baseUri = rtrim((string) \GuztiaConsulting\Aliyun_OSS_Media\Gcp\GuzzleHttp\Psr7\Uri::fromParts($uriParts), '/') . '/';
                return;
            }
        }
    ...
    public function build($resource, $method, array $options = [])
    {
        ...
        return new \GuztiaConsulting\Aliyun_OSS_Media\Gcp\GuzzleHttp\Psr7\Request($action['httpMethod'], $uri, ['Content-Type' => 'application/json'], $body ? $this->jsonEncode($body) : null);
    }
    /**
     * @param string $servicePath
     * @return array
