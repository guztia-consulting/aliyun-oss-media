...
trait TimeTrait
{
    ...
    private function formatTimeAsString(\DateTimeInterface $dateTime, $ns)
    {
        ...
        if ($ns === null) {
            return $dateTime->format(\GuztiaConsulting\Aliyun_OSS_Media\Gcp\Google\Cloud\Core\Timestamp::FORMAT);
        } else {
            return sprintf($dateTime->format(\GuztiaConsulting\Aliyun_OSS_Media\Gcp\Google\Cloud\Core\Timestamp::FORMAT_INTERPOLATE), $this->convertNanoSecondsToFraction($ns));
        }
    }
    /**
