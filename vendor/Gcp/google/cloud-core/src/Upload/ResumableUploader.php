...
use GuztiaConsulting\Aliyun_OSS_Media\Gcp\Google\Cloud\Core\Exception\UploadException;
...
use GuztiaConsulting\Aliyun_OSS_Media\Gcp\GuzzleHttp\Psr7\LimitStream;
 ...
 */
class ResumableUploader extends \GuztiaConsulting\Aliyun_OSS_Media\Gcp\Google\Cloud\Core\Upload\AbstractUploader
{
    use JsonTrait;
    /**
     * @var callable
     ...
     * }
     */
    public function __construct(\GuztiaConsulting\Aliyun_OSS_Media\Gcp\Google\Cloud\Core\RequestWrapper $requestWrapper, $data, $uri, array $options = [])
    {
    ...
    public function resume($resumeUri)
    {
        if (!$this->data->isSeekable()) {
            throw new \GuztiaConsulting\Aliyun_OSS_Media\Gcp\Google\Cloud\Core\Exception\GoogleException('Cannot resume upload on a stream which cannot be seeked.');
        }
    ...
    public function upload()
    {
        ...
        do {
            $data = new \GuztiaConsulting\Aliyun_OSS_Media\Gcp\GuzzleHttp\Psr7\LimitStream($this->data, $this->chunkSize ?: -1, $rangeStart);
            ...
            $request = new \GuztiaConsulting\Aliyun_OSS_Media\Gcp\GuzzleHttp\Psr7\Request('PUT', $resumeUri, $headers, $data);
            try {
                $response = $this->requestWrapper->send($request, $this->requestOptions);
            } catch (GoogleException $ex) {
                throw new \GuztiaConsulting\Aliyun_OSS_Media\Gcp\Google\Cloud\Core\Exception\ServiceException("Upload failed. Please use this URI to resume your upload: {$this->resumeUri}", $ex->getCode(), null, json_decode($ex->getMessage(), true) ?: []);
            }
    ...
    {
        throw new \GuztiaConsulting\Aliyun_OSS_Media\Gcp\Google\Cloud\Core\Exception\GoogleException('Currently only the MultiPartUploader supports async.');
    }
    /**
     ...
     * @return array
     */
    protected function decodeResponse(\GuztiaConsulting\Aliyun_OSS_Media\Gcp\Psr\Http\Message\ResponseInterface $response)
    {
    ...
    protected function createResumeUri()
    {
        ...
        $request = new \GuztiaConsulting\Aliyun_OSS_Media\Gcp\GuzzleHttp\Psr7\Request('POST', $this->uri, $headers, $body);
        $response = $this->requestWrapper->send($request, $this->requestOptions);
    ...
    {
        $request = new \GuztiaConsulting\Aliyun_OSS_Media\Gcp\GuzzleHttp\Psr7\Request('PUT', $this->resumeUri, ['Content-Range' => 'bytes */*']);
