...
class StorageObject
{
     ...
     * @deprecated
     */
    const DEFAULT_DOWNLOAD_URL = \GuztiaConsulting\Aliyun_OSS_Media\Gcp\Google\Cloud\Storage\SigningHelper::DEFAULT_DOWNLOAD_HOST;
    /**
     ...
     */
    public function __construct(\GuztiaConsulting\Aliyun_OSS_Media\Gcp\Google\Cloud\Storage\Connection\ConnectionInterface $connection, $name, $bucket, $generation = null, array $info = [], $encryptionKey = null, $encryptionKeySHA256 = null)
    {
        ...
        $this->acl = new \GuztiaConsulting\Aliyun_OSS_Media\Gcp\Google\Cloud\Storage\Acl($this->connection, 'objectAccessControls', $this->identity);
    }
    /**
    ...
    public function copy($destination, array $options = [])
    {
        ...
        return new \GuztiaConsulting\Aliyun_OSS_Media\Gcp\Google\Cloud\Storage\StorageObject($this->connection, $response['name'], $response['bucket'], $response['generation'], $response + ['requesterProjectId' => $this->identity['userProject']], $key, $keySHA256);
    }
    /**
    ...
    public function rewrite($destination, array $options = [])
    {
        ...
        return new \GuztiaConsulting\Aliyun_OSS_Media\Gcp\Google\Cloud\Storage\StorageObject($this->connection, $response['resource']['name'], $response['resource']['bucket'], $response['resource']['generation'], $response['resource'] + ['requesterProjectId' => $this->identity['userProject']], $destinationKey, $destinationKeySHA256);
    }
    /**
     * Renames the object.
     *
    ...
    {
        $destination = \GuztiaConsulting\Aliyun_OSS_Media\Gcp\GuzzleHttp\Psr7\stream_for(fopen($path, 'w'));
        \GuztiaConsulting\Aliyun_OSS_Media\Gcp\GuzzleHttp\Psr7\copy_to_stream($this->downloadAsStream($options), $destination);
        $destination->seek(0);
        return $destination;
    }
    /**
    ...
    public function beginSignedUploadSession(array $options = [])
    {
        ...
        $uploader = new \GuztiaConsulting\Aliyun_OSS_Media\Gcp\Google\Cloud\Core\Upload\SignedUrlUploader($this->connection->requestWrapper(), '', $startUri, $uploaderOptions);
