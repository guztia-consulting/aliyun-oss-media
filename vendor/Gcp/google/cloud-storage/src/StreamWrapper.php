...
use GuztiaConsulting\Aliyun_OSS_Media\Gcp\Google\Cloud\Storage\Bucket;
use GuztiaConsulting\Aliyun_OSS_Media\Gcp\GuzzleHttp\Psr7\CachingStream;
...
class StreamWrapper
{
     ...
     */
    public static function register(\GuztiaConsulting\Aliyun_OSS_Media\Gcp\Google\Cloud\Storage\StorageClient $client, $protocol = null)
    {
        $protocol = $protocol ?: self::DEFAULT_PROTOCOL;
        if (!in_array($protocol, stream_get_wrappers())) {
            if (!stream_wrapper_register($protocol, \GuztiaConsulting\Aliyun_OSS_Media\Gcp\Google\Cloud\Storage\StreamWrapper::class, STREAM_IS_URL)) {
    ...
    public function stream_open($path, $mode, $flags, &$openedPath)
    {
        ...
        if ($mode == 'w') {
            $this->stream = new \GuztiaConsulting\Aliyun_OSS_Media\Gcp\Google\Cloud\Storage\WriteStream(null, $options);
            ...
            try {
                ...
                $this->stream = new \GuztiaConsulting\Aliyun_OSS_Media\Gcp\Google\Cloud\Storage\ReadStream($this->bucket->object($this->file)->downloadAsStream($options));
                // Wrap the response in a caching stream to make it seekable
                if (!$this->stream->isSeekable() && $flags & STREAM_MUST_SEEK) {
                    $this->stream = new \GuztiaConsulting\Aliyun_OSS_Media\Gcp\GuzzleHttp\Psr7\CachingStream($this->stream);
                }
     ...
     * @return array|bool
     */
    private function urlStatDirectory(\GuztiaConsulting\Aliyun_OSS_Media\Gcp\Google\Cloud\Storage\StorageObject $object)
    {
        $stats = [];
        $info = $object->info();
