...
trait EncryptionTrait
{
    ...
    {
        $signature = '';
        if (class_exists(\GuztiaConsulting\Aliyun_OSS_Media\Gcp\phpseclib\Crypt\RSA::class) && !$forceOpenssl) {
            $rsa = new \GuztiaConsulting\Aliyun_OSS_Media\Gcp\phpseclib\Crypt\RSA();
            $rsa->loadKey($privateKey);
            $rsa->setSignatureMode(\GuztiaConsulting\Aliyun_OSS_Media\Gcp\phpseclib\Crypt\RSA::SIGNATURE_PKCS1);
            $rsa->setHash('sha256');
