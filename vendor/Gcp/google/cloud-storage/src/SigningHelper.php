...
class SigningHelper
{
     ...
     */
    public function sign(\GuztiaConsulting\Aliyun_OSS_Media\Gcp\Google\Cloud\Storage\Connection\ConnectionInterface $connection, $expires, $resource, $generation, array $options)
    {
     ...
     *        and OpenSSL is not available.
     */
    public function v2Sign(\GuztiaConsulting\Aliyun_OSS_Media\Gcp\Google\Cloud\Storage\Connection\ConnectionInterface $connection, $expires, $resource, $generation, array $options)
    {
     ...
     *        and OpenSSL is not available.
     */
    public function v4Sign(\GuztiaConsulting\Aliyun_OSS_Media\Gcp\Google\Cloud\Storage\Connection\ConnectionInterface $connection, $expires, $resource, $generation, array $options)
    {
        list($credentials, $options) = $this->getSigningCredentials($connection, $options);
        $expires = $this->normalizeExpiration($expires);
        list($resource, $bucket) = $this->normalizeResource($resource);
     ...
     *        (array) `fields` keys.
     */
    public function v4PostPolicy(\GuztiaConsulting\Aliyun_OSS_Media\Gcp\Google\Cloud\Storage\Connection\ConnectionInterface $connection, $expires, $resource, array $options = [])
    {
        list($credentials, $options) = $this->getSigningCredentials($connection, $options);
        $expires = $this->normalizeExpiration($expires);
     ...
     */
    private function getSigningCredentials(\GuztiaConsulting\Aliyun_OSS_Media\Gcp\Google\Cloud\Storage\Connection\ConnectionInterface $connection, array $options)
    {
        ...
        if ($keyFile) {
            ...
            $credentials = \GuztiaConsulting\Aliyun_OSS_Media\Gcp\Google\Auth\CredentialsLoader::makeCredentials($scopes, $keyFile);
        } else {
