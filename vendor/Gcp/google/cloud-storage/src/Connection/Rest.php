 ...
 */
namespace GuztiaConsulting\Aliyun_OSS_Media\Gcp\Google\Cloud\Storage\Connection;

use GuztiaConsulting\Aliyun_OSS_Media\Gcp\Google\Cloud\Core\RequestBuilder;
use GuztiaConsulting\Aliyun_OSS_Media\Gcp\Google\Cloud\Core\RequestWrapper;
use GuztiaConsulting\Aliyun_OSS_Media\Gcp\Google\Cloud\Core\RestTrait;
use GuztiaConsulting\Aliyun_OSS_Media\Gcp\Google\Cloud\Core\Upload\AbstractUploader;
use GuztiaConsulting\Aliyun_OSS_Media\Gcp\Google\Cloud\Core\Upload\MultipartUploader;
...
use GuztiaConsulting\Aliyun_OSS_Media\Gcp\Google\Cloud\Core\UriTrait;
use GuztiaConsulting\Aliyun_OSS_Media\Gcp\Google\Cloud\Storage\Connection\ConnectionInterface;
use GuztiaConsulting\Aliyun_OSS_Media\Gcp\Google\Cloud\Storage\StorageClient;
use GuztiaConsulting\Aliyun_OSS_Media\Gcp\Google\CRC32\Builtin;
use GuztiaConsulting\Aliyun_OSS_Media\Gcp\Google\CRC32\CRC32;
 ...
 */
class Rest implements \GuztiaConsulting\Aliyun_OSS_Media\Gcp\Google\Cloud\Storage\Connection\ConnectionInterface
{
    use RestTrait;
    use UriTrait;
    /**
    ...
    {
        $config += ['serviceDefinitionPath' => __DIR__ . '/ServiceDefinition/storage-v1.json', 'componentVersion' => \GuztiaConsulting\Aliyun_OSS_Media\Gcp\Google\Cloud\Storage\StorageClient::VERSION, 'apiEndpoint' => self::DEFAULT_API_ENDPOINT];
        $this->apiEndpoint = $this->getApiEndpoint(self::DEFAULT_API_ENDPOINT, $config);
        $this->setRequestWrapper(new \GuztiaConsulting\Aliyun_OSS_Media\Gcp\Google\Cloud\Core\RequestWrapper($config));
        $this->setRequestBuilder(new \GuztiaConsulting\Aliyun_OSS_Media\Gcp\Google\Cloud\Core\RequestBuilder($config['serviceDefinitionPath'], $this->apiEndpoint));
    ...
    {
        list($request, $requestOptions) = $this->buildDownloadObjectParams($args);
        return $this->requestWrapper->sendAsync($request, $requestOptions)->then(function (\GuztiaConsulting\Aliyun_OSS_Media\Gcp\Psr\Http\Message\ResponseInterface $response) {
    ...
    public function insertObject(array $args = [])
    {
        ...
        $uploadType = \GuztiaConsulting\Aliyun_OSS_Media\Gcp\Google\Cloud\Core\Upload\AbstractUploader::UPLOAD_TYPE_RESUMABLE;
        if ($args['streamable']) {
            $uploaderClass = \GuztiaConsulting\Aliyun_OSS_Media\Gcp\Google\Cloud\Core\Upload\StreamableUploader::class;
        } elseif ($args['resumable']) {
            $uploaderClass = \GuztiaConsulting\Aliyun_OSS_Media\Gcp\Google\Cloud\Core\Upload\ResumableUploader::class;
        } else {
            $uploaderClass = \GuztiaConsulting\Aliyun_OSS_Media\Gcp\Google\Cloud\Core\Upload\MultipartUploader::class;
            $uploadType = \GuztiaConsulting\Aliyun_OSS_Media\Gcp\Google\Cloud\Core\Upload\AbstractUploader::UPLOAD_TYPE_MULTIPART;
        }
    ...
    private function resolveUploadOptions(array $args)
    {
        ...
        $args['data'] = \GuztiaConsulting\Aliyun_OSS_Media\Gcp\GuzzleHttp\Psr7\stream_for($args['data']);
        if ($args['resumable'] === null) {
            $args['resumable'] = $args['data']->getSize() > \GuztiaConsulting\Aliyun_OSS_Media\Gcp\Google\Cloud\Core\Upload\AbstractUploader::RESUMABLE_LIMIT;
        }
        if (!$args['name']) {
        ...
        if ($validate === 'md5') {
            $args['metadata']['md5Hash'] = base64_encode(\GuztiaConsulting\Aliyun_OSS_Media\Gcp\GuzzleHttp\Psr7\hash($args['data'], 'md5', true));
        } elseif ($validate === 'crc32') {
            $args['metadata']['crc32c'] = $this->crcFromStream($args['data']);
        }
        $args['metadata']['name'] = $args['name'];
        unset($args['name']);
        $args['contentType'] = isset($args['metadata']['contentType']) ? $args['metadata']['contentType'] : \GuztiaConsulting\Aliyun_OSS_Media\Gcp\GuzzleHttp\Psr7\mimetype_from_filename($args['metadata']['name']);
    ...
    private function buildDownloadObjectParams(array $args)
    {
        ...
        return [new \GuztiaConsulting\Aliyun_OSS_Media\Gcp\GuzzleHttp\Psr7\Request('GET', \GuztiaConsulting\Aliyun_OSS_Media\Gcp\GuzzleHttp\Psr7\uri_for($uri)), $requestOptions];
    }
    /**
     ...
     * @return string
     */
    private function crcFromStream(\GuztiaConsulting\Aliyun_OSS_Media\Gcp\Psr\Http\Message\StreamInterface $data)
    {
        $pos = $data->tell();
        if ($pos > 0) {
            $data->rewind();
        }
        $crc32c = \GuztiaConsulting\Aliyun_OSS_Media\Gcp\Google\CRC32\CRC32::create(\GuztiaConsulting\Aliyun_OSS_Media\Gcp\Google\CRC32\CRC32::CASTAGNOLI);
        $data->rewind();
        while (!$data->eof()) {
    ...
    {
        return \GuztiaConsulting\Aliyun_OSS_Media\Gcp\Google\CRC32\Builtin::supports(\GuztiaConsulting\Aliyun_OSS_Media\Gcp\Google\CRC32\CRC32::CASTAGNOLI);
    }
}
