...
use DateTime;
use Exception;
use GuztiaConsulting\Aliyun_OSS_Media\Gcp\Firebase\JWT\ExpiredException;
use GuztiaConsulting\Aliyun_OSS_Media\Gcp\Firebase\JWT\JWT;
use GuztiaConsulting\Aliyun_OSS_Media\Gcp\Firebase\JWT\SignatureInvalidException;
use GuztiaConsulting\Aliyun_OSS_Media\Gcp\Google\Auth\Cache\MemoryCacheItemPool;
...
use GuztiaConsulting\Aliyun_OSS_Media\Gcp\phpseclib\Crypt\RSA;
use GuztiaConsulting\Aliyun_OSS_Media\Gcp\phpseclib\Math\BigInteger;
use GuztiaConsulting\Aliyun_OSS_Media\Gcp\Psr\Cache\CacheItemPoolInterface;
use RuntimeException;
use GuztiaConsulting\Aliyun_OSS_Media\Gcp\SimpleJWT\InvalidTokenException;
use GuztiaConsulting\Aliyun_OSS_Media\Gcp\SimpleJWT\JWT as SimpleJWT;
use GuztiaConsulting\Aliyun_OSS_Media\Gcp\SimpleJWT\Keys\KeyFactory;
use GuztiaConsulting\Aliyun_OSS_Media\Gcp\SimpleJWT\Keys\KeySet;
...
class AccessToken
{
     ...
     */
    public function __construct(callable $httpHandler = null, \GuztiaConsulting\Aliyun_OSS_Media\Gcp\Psr\Cache\CacheItemPoolInterface $cache = null)
    {
        $this->httpHandler = $httpHandler ?: \GuztiaConsulting\Aliyun_OSS_Media\Gcp\Google\Auth\HttpHandler\HttpHandlerFactory::build(\GuztiaConsulting\Aliyun_OSS_Media\Gcp\Google\Auth\HttpHandler\HttpClientCache::getHttpClient());
        $this->cache = $cache ?: new \GuztiaConsulting\Aliyun_OSS_Media\Gcp\Google\Auth\Cache\MemoryCacheItemPool();
    }
    /**
    ...
    {
        $this->checkSimpleJwt();
        $jwkset = new \GuztiaConsulting\Aliyun_OSS_Media\Gcp\SimpleJWT\Keys\KeySet();
        foreach ($certs as $cert) {
            $jwkset->add(\GuztiaConsulting\Aliyun_OSS_Media\Gcp\SimpleJWT\Keys\KeyFactory::create($cert, 'php'));
        }
    ...
    private function verifyRs256($token, array $certs, $audience = null, $issuer = null)
    {
        ...
        foreach ($certs as $cert) {
            ...
            $rsa = new \GuztiaConsulting\Aliyun_OSS_Media\Gcp\phpseclib\Crypt\RSA();
            $rsa->loadKey(['n' => new \GuztiaConsulting\Aliyun_OSS_Media\Gcp\phpseclib\Math\BigInteger($this->callJwtStatic('urlsafeB64Decode', [$cert['n']]), 256), 'e' => new \GuztiaConsulting\Aliyun_OSS_Media\Gcp\phpseclib\Math\BigInteger($this->callJwtStatic('urlsafeB64Decode', [$cert['e']]), 256)]);
    ...
    public function revoke($token, array $options = [])
    {
        ...
        $body = \GuztiaConsulting\Aliyun_OSS_Media\Gcp\GuzzleHttp\Psr7\stream_for(http_build_query(['token' => $token]));
        $request = new \GuztiaConsulting\Aliyun_OSS_Media\Gcp\GuzzleHttp\Psr7\Request('POST', self::OAUTH2_REVOKE_URI, ['Cache-Control' => 'no-store', 'Content-Type' => 'application/x-www-form-urlencoded'], $body);
    ...
    private function retrieveCertsFromLocation($url, array $options = [])
    {
        ...
        $httpHandler = $this->httpHandler;
        $response = $httpHandler(new \GuztiaConsulting\Aliyun_OSS_Media\Gcp\GuzzleHttp\Psr7\Request('GET', $url), $options);
    ...
    private function checkAndInitializePhpsec()
    {
        ...
        if (!class_exists('GuztiaConsulting\\Aliyun_OSS_Media\\Gcp\\phpseclib\\Crypt\\RSA')) {
    ...
    {
        // @codeCoverageIgnoreStart
        if (!class_exists('GuztiaConsulting\\Aliyun_OSS_Media\\Gcp\\SimpleJWT\\JWT')) {
    ...
    private function setPhpsecConstants()
    {
        if (filter_var(getenv('GAE_VM'), FILTER_VALIDATE_BOOLEAN)) {
            ...
            if (!defined('CRYPT_RSA_MODE')) {
                define('CRYPT_RSA_MODE', \GuztiaConsulting\Aliyun_OSS_Media\Gcp\phpseclib\Crypt\RSA::MODE_OPENSSL);
            }
        }
    }
    /**
    ...
    {
        $class = class_exists('GuztiaConsulting\\Aliyun_OSS_Media\\Gcp\\Firebase\\JWT\\JWT') ? 'GuztiaConsulting\\Aliyun_OSS_Media\\Gcp\\Firebase\\JWT\\JWT' : 'JWT';
    ...
    {
        return call_user_func_array(['GuztiaConsulting\\Aliyun_OSS_Media\\Gcp\\SimpleJWT\\JWT', 'decode'], $args);
    }
    /**
