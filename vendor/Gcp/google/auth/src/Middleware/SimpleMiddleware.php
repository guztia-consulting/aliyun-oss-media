...
class SimpleMiddleware
{
    ...
    public function __invoke(callable $handler)
    {
        return function (\GuztiaConsulting\Aliyun_OSS_Media\Gcp\Psr\Http\Message\RequestInterface $request, array $options) use($handler) {
            ...
            $query = \GuztiaConsulting\Aliyun_OSS_Media\Gcp\GuzzleHttp\Psr7\parse_query($request->getUri()->getQuery());
            $params = array_merge($query, $this->config);
            $uri = $request->getUri()->withQuery(\GuztiaConsulting\Aliyun_OSS_Media\Gcp\GuzzleHttp\Psr7\build_query($params));
