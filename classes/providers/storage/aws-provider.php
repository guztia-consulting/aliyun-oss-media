...
use GuztiaConsulting\Aliyun_OSS_Media\Providers\Storage\Streams\AWS_S3_Stream_Wrapper;
...
class AWS_Provider extends Storage_Provider {
	...
	private function batch_command( $command, array $items ) {
		...
		if ( ! empty( $results ) ) {
			foreach ( $results as $result ) {
				...
				if ( is_a( $result, 'GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\S3\Exception\S3Exception' ) ) {
	...
	public function list_keys( array $locations ) {
		...
		foreach ( $results as $attachment_id => $result ) {
			if ( is_a( $result, 'GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\ResultInterface' ) ) {
				$found_keys = $result->search( 'Contents[].Key' );

				if ( ! empty( $found_keys ) ) {
					$keys[ $attachment_id ] = $found_keys;
				}
			} elseif ( is_a( $result, 'GuztiaConsulting\Aliyun_OSS_Media\Aws3\Aws\S3\Exception\S3Exception' ) ) {
				/* @var S3Exception $result */
